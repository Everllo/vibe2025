...
async function addListItem(itemText) {
    try {
        ...
        const [result] = await connection.execute(query, [itemText]);
        await connection.end();
...
async function updateListItem(itemId, newText) {
    try {
        const connection = await mysql.createConnection(dbConfig);
        const query = 'UPDATE items SET text = ? WHERE id = ?';
        const [result] = await connection.execute(query, [newText, itemId]);
        await connection.end();
    ...
    if (req.url === '/' && req.method === 'GET') {
        ...
        req.on('end', async () => {
            try {
                ...
                const newId = await addListItem(text);
